...
$id= $_GET['id'];
$sql = $mysqli->query("SELECT * FROM categories WHERE category_id='$id'");
 $row = $sql->fetch_assoc();

if (isset($_POST['submit'])) {
 $category_name = $mysqli->escape_string($_POST['category']);

 $update = $mysqli->query("UPDATE categories SET category_name = '$category_name' WHERE category_id='$id'");
}
 ?>
...
<body class="o-page">
  ...
  <main class="o-page__content">
    ...
    <div class="container-fluid">
      ...
      <div class="col-12">
        <div class="card-box">
          <h4 class="m-t-0 header-title">Update Category</h4>
          <form  method="POST" id="update_category" class="update_category" >
              <div class="alert"></div>

            <div class="form-group row">
                <label for="category" class="col-sm-3 col-form-label">Category Name
                  <i class="text-danger"> *</i>
                </label>
              <div class="col-sm-6">

                <input class="form-control" id="category_name" aria-required="true" type="text" name="category" value="<?php echo htmlentities($row['category_name']); ?>">

                </div>
                  </div>
                <div class=" form-group row">
                  <div class="col-sm-6 push-md-3">
                    <input type="submit" id="update_category"  class="btn btn-primary btn-large" name="submit" value="Save">
                  </div>
                </div>
...
/*$('#update_category').on('submit',function(e){
  ...
  $.ajax({
    method:"POST",
    url:"http://localhost/demo/php_actions/updateCategories.php",
